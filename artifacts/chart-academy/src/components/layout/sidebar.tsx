...
export function Sidebar() {
            ...
            return (
              <div key={lib.id} className="px-2">
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-2 text-sm font-semibold text-foreground hover:bg-secondary/50 rounded-lg transition-colors group"
                >
                  ...
                  <ChevronDown className={cn(
                    ...
                    expandedLibs[lib.id] ? "rotate-180" : ""
                  )} />
                </button>
                      ...
                      <div className="pt-1 pb-2 space-y-1 ml-4 border-l border-border pl-2 mt-1">
                        ...
                        {lib.lessons.map((lesson, idx) => {
                          ...
                          return (
                            <Link
                              key={lesson.id}
                              href={`/lesson/${lib.id}/${lesson.id}`}
                              onClick={closeMobile}
                              className={cn(
                                "flex items-start space-x-2 px-3 py-1.5 text-sm rounded-md transition-colors",
                                isLessonActive
                                  ? "bg-primary/10 text-primary"
                                  : "text-muted-foreground hover:text-foreground hover:bg-secondary/50"
                              )}
  ...
  return (
    <>
        ...
        {isOpen && (
          <>
            ...
            <motion.div 
              ...
              className="fixed inset-y-0 left-0 w-4/5 max-w-[300px] z-50 md:hidden shadow-2xl"
            >
              {navContent}
              <button 
                onClick={closeMobile}
      ...
      {/* Desktop Sidebar */}
      <div className="hidden md:block h-screen sticky top-0 left-0 z-30">
        {navContent}
      </div>
    </>
  );
