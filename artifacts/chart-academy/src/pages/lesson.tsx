...
export default function Lesson() {
  const { libId, lessonId } = useParams();
  ...
  return (
    <div className="h-[calc(100vh-64px)] md:h-screen flex flex-col md:flex-row overflow-hidden bg-background">
      ...
      <div className={`md:flex flex-col w-full md:w-1/3 lg:w-[30%] border-r border-border bg-card overflow-y-auto ${activeTab === 'theory' ? 'flex' : 'hidden'}`}>
        ...
        <div className="mt-auto p-6 bg-secondary/30 border-t border-border mt-8">
          <button
            ...
            className="w-full py-3 px-4 rounded-xl flex items-center justify-center space-x-2 font-semibold transition-all duration-200 shadow-sm
              bg-gradient-to-r from-primary to-primary/80 text-primary-foreground hover:shadow-lg hover:shadow-primary/20 hover:-translate-y-0.5"
          >
      ...
      <div className={`md:flex flex-col w-full md:w-1/3 lg:w-[35%] border-r border-border bg-background ${activeTab === 'editor' ? 'flex' : 'hidden'}`}>
        <div className="flex items-center justify-between px-4 py-3 bg-card border-b border-border shrink-0">
          ...
          <div className="flex items-center space-x-2">
            <button 
              onClick={handleReset}
              className="p-1.5 text-muted-foreground hover:text-foreground rounded-md hover:bg-secondary transition-colors tooltip-trigger"
              title="Reset Code"
            >
            ...
            <button 
              onClick={handleRun}
              className="flex items-center space-x-1.5 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 px-3 py-1.5 rounded-md text-xs font-bold uppercase tracking-wider transition-colors border border-emerald-500/20"
            >
