      ...
      <div className="bg-card border border-border rounded-3xl p-8 md:p-10 mb-12 shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-4 max-w-2xl">
            ...
            <div className="flex items-center space-x-4 text-sm font-medium text-muted-foreground">
              ...
              <span className="flex items-center space-x-1">
                ...
                <span>{library.lessons.length} Interactive Lessons</span>
              </span>
            </div>
          </div>
          ...
          <div className="shrink-0 w-full md:w-64 bg-secondary/50 rounded-2xl p-5 border border-border">
            ...
            {nextLesson ? (
              <Link 
                href={`/lesson/${library.id}/${nextLesson.id}`}
                className="w-full flex items-center justify-center space-x-2 bg-primary hover:bg-primary/90 text-primary-foreground py-2.5 rounded-xl font-medium transition-colors shadow-sm shadow-primary/20"
              >
                <Play className="w-4 h-4" />
                <span>{progress === 0 ? "Start Course" : "Continue"}</span>
              </Link>
            ) : (
          ...
          {library.lessons.map((lesson, idx) => {
            ...
            return (
              <motion.div key={lesson.id} variants={item}>
                <Link
                  href={`/lesson/${library.id}/${lesson.id}`}
